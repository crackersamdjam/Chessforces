...
/** Enemy HQ cells to bias toward in 2v2. */
const ENEMY_HQ = {
	N: [
		{ r: 7, c: 0 },
		{ r: 9, c: 0 },
...
export function findLegalPlayMoves(room, seat, { limit = 64, biasToEnemyHq = false } = {}) {
	...
	const hqTargets = biasToEnemyHq ? (ENEMY_HQ[seat] ?? []) : [];
	const distToEnemy = (r, c) => {
		if (!hqTargets.length) return 0;
		return Math.min(...hqTargets.map((t) => Math.abs(t.r - r) + Math.abs(t.c - c)));
	};
